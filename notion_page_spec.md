...
**Version:** 1.1  
**Date:** 2026-03-28  
**Scope:** `src/adapter/writeback/notion/client.rs` および関連モジュールの Notion 個人ページ write-back 出力フォーマットの再設計  
**Status:** Draft — Post-MVP（M07 Write-Back）。現行 MVP は `Command` / `EffectPlan` 定義まで。本仕様の実装は M07 着手後に行う。
...
本仕様の実装が遵守すべき law（`plan.md` §0.4 / `domain_algebra.md` §7）：
...
データの取得元である `StudentProfile`（`src/slide_analysis/types.rs`）、`PersonProfile`（`src/person_page/types.rs`）、`FrontendProfile` の型定義には変更を加えない。変更対象は write-back アダプタが `WriteRecord` の `payload` を Notion API コールに変換する部分のみである。

### 関連ファイル（読み取り専用・変更不要）

```
src/slide_analysis/types.rs        — StudentProfile, StudentProperties, ProfilePic, GalleryImage
src/person_page/types.rs           — PersonProfile, FrontendProfile, PersonActivity
src/person_page/projector.rs       — PersonPageProjector
src/adapter/writeback/traits.rs    — SaaSWriteAdapter, WriteRecord, WriteResult
src/slide_analysis/gemini.rs       — GeminiSlideAnalyzer (Gemini プロンプト)
src/governance/mod.rs              — PolicyEngine, FilteringGate, AuditLog
```

### 変更対象ファイル

```
src/adapter/writeback/notion/client.rs — NotionWriteAdapter の実装（主な変更箇所）
src/slide_analysis/gemini.rs           — Gemini プロンプトへの profile_pic 選定指示の追加
```
...
**⚠️ Confidence Gate（`governance_capability_model.md` §3.5 準拠）:**
...
`src/slide_analysis/gemini.rs` の `extract_profile_from_png` メソッド内のプロンプトに以下の指示を追加する。
...
Gemini の出力は Supplemental derivation（reusable but non-canonical）に分類される（`domain_algebra.md` §4.1）。Gemini が返す `profile_pic` / `gallery_images` は自動的に Lake 正史にはならない。
...
新規モジュール `src/adapter/writeback/image_crop.rs` を作成する。
...
**Filtering-before-Exposure:** クロッピングされた画像（特に顔写真）は incidental capture に該当しうる（`governance_capability_model.md` §3.4）。公開 URL を生成する前に以下を確認する：
...
`src/adapter/writeback/notion.rs` に以下の関数を実装する。
...
### 8.1 ユニットテスト (`src/adapter/writeback/notion.rs`)
...
### 8.2 クロッピングテスト (`src/adapter/writeback/image_crop.rs`)
...
| Phase | 作業内容 | 変更ファイル |
|---|---|---|
| **Phase 1** | `build_page_blocks` 関数の実装 + ユニットテスト | `src/adapter/writeback/notion/client.rs` |
| **Phase 2** | `NotionWriteAdapter::write_record` 内で `build_page_blocks` を使用。properties の書き込みを §1 に従い更新。cover 設定を追加 | `src/adapter/writeback/notion/client.rs` |
| **Phase 3** | `FilteringGate` 統合。write-back 前の policy check、Visibility 決定ロジック、AuditLog 記録を実装。governance テスト追加 | `src/adapter/writeback/notion/client.rs`, `src/governance/mod.rs` |
| **Phase 4** | `image_crop.rs` の新規作成。`Cargo.toml` に `image` crate 追加。icon 設定ロジック + confidence gate を追加 | `src/adapter/writeback/image_crop.rs`, `Cargo.toml`, `src/adapter/writeback/mod.rs`, `src/adapter/writeback/notion/client.rs` |
| **Phase 5** | Gemini プロンプトに `profile_pic` 選定ガイダンスを追加 | `src/slide_analysis/gemini.rs` |
...
2. 429 レスポンスに対しては `Retry-After` ヘッダを尊重して待機する（既存の `src/adapter/retry.rs` のリトライポリシーを使用）
