...
- **D9.2 SQLite-authoritative per leaf。** `observations` は `leaf_id` と leaf-local
  `append_seq` を持つ SQLite authoritative store とする。in-memory `LakeStore` は
  現プロセスで取り込んだデータの非永続キャッシュに限定し、起動時は永続
  Projection materialization から復元して Observation 全件を RAM 展開しない。
  dedup 判定は SQLite の `UNIQUE (leaf_id, identity_key)` に一本化し、持続層では
  補償ロールバックを使用しない。
