...
この文書は、`system-overview.md` の意味論的な核を補強するための補助仕様です。
主に **型、law、失敗モデル、write の正規化、storage の意味境界** を扱います。

`system-overview.md` が全体像と各レイヤの関係を説明する親文書であるのに対し、この文書は「何を純粋関数として扱い、どこで副作用を起こし、どの条件を満たせば設計が壊れないか」を明示します。
...
| **ApplicationCached** | **Projection cache のみ** | cache miss は明示エラー |

source-native、Lake snapshot、Projection cache は spec に明記された入力だけを使う。取得失敗や cache miss を別経路で補完してはならず、明示エラーにする。

source-native を読んだ場合は、lineage に `sourceNativeRead { system, timestamp, revision }` と入力参照を記録する。revision を取得できない source は deterministic build の入力にできない。Lake と source-native を同一 Projection で併用する場合は、`lake-first` / `source-latest` / `dual-track` の reconciliation policy を宣言し、未宣言なら registration を拒否する。
...
6. **Idempotency Law**  
   ...
   sharding 下では per-leaf exact index による**完全（決定的）冪等**に強化する（確率版は撤回）。idempotencyKey は per-message の `source:object_id:H(canonical_content)` で必須・解決可能。詳細は [Sharding design](sharding.md)。
...
詳細は [ADR backlog](../decisions/adr-backlog.md) を参照してください。
