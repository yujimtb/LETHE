...
履歴Observationのv2 identityは、履歴固有の決定的な
`object_id = source_session_id:source_message_id`、既存の履歴payloadを文字列化した
`meta.canonical_json`、および安定したunitである`meta.source_instance`から、次の公式
形式で生成します。

```text
source_instance:object_id:sha256(canonical_json)
```

`meta.object_id`、`meta.source_instance`、`meta.canonical_json`、およびこの
idempotency keyはretry間で変更してはいけません。旧版の
`history-message:<sha256(identity:raw_sha256)>`と`meta.source_instance_id`だけを持つ
既存行は削除・更新せず、SQLiteのidentity bridgeが履歴payload内の
`source_session_id` / `source_message_id`を決定論的に読み取りcandidateへ投影します。
bridgeのexact canonical matchは旧Observation IDのduplicateへ解決され、ledger deltaは
0になります。必須原料が欠ける行は推測せずgapとしてactivationをblockします。
...
### v2 bridgeと直接append admission

`lethe-import-history`はHTTPのImportApiClientへ置き換えず、SQLite Personal Lakeの
Operational EventとObservationを同じtransactionで直接appendします。各message batchの
前にbridgeの未処理tailだけをboundedにcatch-upし、旧
`history-message:` Observationがあればcandidateの最小`append_seq`を既存IDとして
返します。canonical JSONのexact mismatchは新規appendせずエラーで停止します。

cutover登録済みunitへ実行する場合は、source instanceごとにv2 generationを明示して
ください。同じunitを複数source kindが参照してもgenerationは1つです。

```text
--admission-generation=claude-code-personal=2
--admission-generation=codex-personal=2
```

`v2_active` / `v2_committed` unitでgenerationを省略、またはstale generationを指定した
場合は、message handlerへ到達せず明示的なcutover admission errorで停止します。
未登録unitはgenerationなしで実行できますが、登録済み`v1_active` / `draining` unitへ
v2 messageを送ることはできません。v2で最初の`ingested`が発生したunitは
`v2_committed`となり、以後のrollbackは拒否されます。

同じ契約を使う`crates/history`の`HistoryImportCommand`とHTTP history importでは、
`admission_generations` mapにsource instanceごとのgenerationを渡します。未登録unitに
ついてはmapへ登録せず、登録済みv2 unitのmap漏れはstorage側で明示的に拒否します。

この直接bridge/admission経路は現在SQLite Personal Lake専用です。CLIのPostgreSQL
backendへ暗黙に旧appendするfallbackはなく、v2履歴messageの実行は明示的なunsupported
errorで停止します。cutover register、drain、bridge watermark、zero-gap、fixture
...
SQLiteのv2直接appendはbatch内のmessage eventとObservationを1 transactionで登録します。
