...
実装日: 2026-07-15
対象: 候補C（安定ID/DSUへ進む前の局所再投影段階）

## 実装結果

Slack 増分で identity topology、identifier owner、または consent が変化した場合も、
...
- 無関係componentのObservationは読まない。現行の positional `resolved-N` により
  public ID が変わるcomponentは出力上の affected component として扱う。

`IdentityProjector::resolve_with_component_members` は、resolved person と同じ順序の
candidate membership を返す。増分側はこの結果を利用するため、union規則を別実装
していない。
...
non-corpus materialization formatを `4` から `5` へ上げた。version 5では各
...
ただしこの段階では、全candidate identity resolve、resident manifest clone/serialize、
positional person IDによるsuffix再採番を残す。このため候補C単独の一般上界を
`O((N+C) log C)` とは扱わない。次段の候補Bで stable component ID、normalized
IdentifierKey bucket、append-only DSU、Observation由来stable fact ID、identity
nodeへの間接所属、component aggregateへ置換し、全履歴amortized
`O((N+C) log C)` を目標とする。
