...
実装日: 2026-07-15
対象: 候補C（component局所再投影）と候補Bの安定ID基盤
...
format 6でpositional person IDとperson内ordinal fact IDによる再採番は除去した。
ただし全candidate identity resolve、fact rowのperson owner直接保持、resident manifest
clone/serializeは残る。このため現段階の一般上界を `O((N+C) log C)` とは扱わない。
次段でnormalized IdentifierKey bucket、append-only DSU、identity nodeへのfact間接所属、
component aggregateへ置換し、全履歴amortized
`O((N+C) log C)` を目標とする。
