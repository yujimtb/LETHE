...
- API は projection 汎用 route `/api/projections/{projection_id}/records` を正路とし、`/api/persons/*` は `Deprecation: true` 付き alias とする。
...
- `tests/self_host_api.rs` の未認証拒否・認証付き projection/blob tests
