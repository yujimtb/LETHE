...
canonical 書き込み lane の排他ロックは短時間 SHALL とし、blob I/O・page 走査・network 待ちの間に AppCore lock を保持 SHALL NOT する。読み取り lane は immutable snapshot を `Arc` で公開して lock なしで参照 SHALL し、SQLite は read connection pool・PostgreSQL は connection pool を書き込みと分離 SHALL する。
