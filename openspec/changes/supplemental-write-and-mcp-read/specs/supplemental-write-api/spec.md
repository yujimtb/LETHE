...
**Module:** M14 API Serving(拡張)+ M04 Supplemental Store(契約昇格、実装変更なし)
...
selfhost は `POST /supplementals` を提供 SHALL する。リクエストボディは SupplementalRecord の JSON 表現(id, kind, derived_from, payload, created_by, mutability, model_version, consent_metadata, lineage)。成功時は 201 と格納済みレコードのエンベロープを返す。
...
- **WHEN** verification_mode のない payload を kind `claim` で POST する
