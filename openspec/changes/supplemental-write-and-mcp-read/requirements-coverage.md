...
| MCP read/write port | Pass | E2E covers listener separation, OAuth JWT validation, metadata, read/write tool descriptors, per-tool security schemes, projection-only reads, and `write_supplemental` authorization. Live Tailscale Funnel exposes only the MCP port, Auth0 DCR/JWT smoke passed, claude.ai, ChatGPT.com, Claude Code, and Codex all returned live `search_lake` data, and ChatGPT.com wrote a supplemental decision through the public MCP endpoint. |
...
| MCPR-02: Streamable HTTP transport is implemented. | Pass for live public endpoint | `apps/selfhost/src/self_host/mcp.rs`; test `six_mcp_tools_have_contracts_and_read_via_projection`; live public `POST https://yujiws.tail474356.ts.net/mcp` returned a valid JSON-RPC `tools/list` result with an Auth0-issued JWT. |
| MCPR-03: OAuth resource-server metadata and JWT validation; no token issuance/API key auth. | Pass | `mcp.rs`, `mcp_contract.rs`; tests `protected_resource_metadata_contract_is_public`, `mcp_jwt_validation_rejects_expired_and_wrong_audience_and_accepts_valid`, `mcp_jwt_validation_accepts_auth0_permissions_claim_for_refreshed_tokens`; Auth0 tenant `lethe-mcp.jp.auth0.com` has API `LETHE MCP Read Port` with identifier/audience `https://yujiws.tail474356.ts.net/mcp`, DCR and offline access enabled, public JWKS loaded locally, tokenless public `/mcp` returned 401 + `WWW-Authenticate` with `scope="mcp:read write:supplemental"`, and Auth0-issued JWT returned 6 tools. |
| MCPR-04: five read tools and one `write_supplemental` tool are exposed with per-tool OAuth schemes. | Pass | `mcp.rs`; test `six_mcp_tools_have_contracts_and_read_via_projection` asserts names, descriptions, read/write annotations, `securitySchemes`, and `_meta.securitySchemes`; `write_supplemental_requires_write_scope_and_refreshes_projection` asserts read-only tokens receive a tool-level `mcp/www_authenticate` challenge for `write:supplemental`. |
...
- 2026-07-08 refresh-token reauthorization evidence: Auth0 third-party default permissions were updated to `mcp:read` plus `write:supplemental`; stale failed-attempt DCR clients were deleted; claude.ai, ChatGPT.com, Claude Code, and Codex CLI each called the public MCP endpoint with `query = "aquisition"`, `source_types = ["github-commit"]`, and `limit = 1`, returning `corpus:github-commit:019f35ff-3750-7721-8748-326adacde778`.
- 2026-07-08 ChatGPT.com write evidence: App settings `Refresh` discovered `write_supplemental` as a `WRITE` action requiring `write:supplemental`; `Reconnect` consent included `mcp:read`, `write:supplemental`, and `offline_access`; live ChatGPT wrote `sup:beaf7489-61dd-48bb-8015-068390fb5cc5` and found statement `ChatGPT write_supplemental smoke 2026-07-07T16:03:29Z` through `search_decisions`.
