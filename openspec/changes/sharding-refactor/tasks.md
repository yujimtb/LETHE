...
- [ ] 1.4 衝突判定を `same_idempotent_observation`(full observation 比較) → stored `canonical_json` のみの exact compare に置換(SHARD-04 §canonical タプル衝突判定、R4)
...
- [ ] 1.7 **rehome primitive** を fresh ingest と別レールで実装(SHARD-07)
...
- [ ] 1.8 property test: 編集なし再取り込み → `Duplicate`、編集 → 新 distinct Observation、claude.ai 再 export 欠落 → no-op
...
- [ ] 2.1 Google Slides adapter を canonical タプル宣言契約へ作り直し(revisioned snapshot は維持)(SHARD-ADAPT-01)
- [ ] 2.2 claude.ai zip importer を新規実装(message uuid / 欠落時は conversation_uuid + parent_message_uuid チェーン位置から決定的に導出)(SHARD-ADAPT-02)
- [ ] 2.3 generic adapter framework(object_id 抽出 trait + canonical タプル trait)(SHARD-ADAPT-01)
- [ ] 2.4 adapter conformance test: 同 source の reactions / 編集 wrapper 変化で `identity_key` が変わらないこと
- [ ] 2.5 `Observation.idempotencyKey` を optional → 必須・高エントロピー・解決可能に格上げ(SHARD-01)
...
- [ ] 3.1 Patricia trie の永続表現(`tree(L)` = `initialize` + `split_commit` の fold)(SHARD-RT-02)
- [ ] 3.2 `routing_key` 計算実装: `coarse(month) : coarse(year) : source : container : fine(published)`(SHARD-RT-01)
...
- [ ] 3.5 partition log events: `split_prepare` / `split_commit` を `event_seq` + optional control-plane timestamp で永続(SHARD-RT-04、R10)
- [ ] 3.6 split 中の crash: `prepare` だけで `commit` なしは `tree(L)` に影響しない(replay test)
- [ ] 3.7 leaf id = 不透明 `lake:<uuid>`、path / 責任ビット範囲は log から計算(SHARD-RT-04)
...
- [ ] 4.1 `candidate_leaves(filter, log)` 実装: routing 軸 prune + published time-window を `(month, year)` bucket 集合に展開して部分木 union(SHARD-RT-06、R3)
- [ ] 4.2 streaming k-way merge by `(published, recorded_at, id)` 実装(SHARD-RT-07)
- [ ] 4.3 resolver を Imperative Shell に配置、Kernel は論理 lake のみを見る(SHARD-RT-07 §Effect Isolation)
- [ ] 4.4 watermark を `projection_watermarks` テーブルから per-(projection × leaf) cursor table へ migrate(SHARD-PROP-01)
- [ ] 4.5 cursor を `recorded_at` / `published` → leaf-local `append_seq` に置換(SHARD-PROP-02、R2)
...
- [ ] 4.8 split 後の全 Observation 再配送を baseline で許容(SHARD-PROP-05、R11)、profiling で β(frontier 子移管)を後付け可能に
- [ ] 4.9 supplemental index は明示宣言の read mode のみ(暗黙 full-scan fallback 禁止)(SHARD-RT-08、R6)
- [ ] 4.10 placement に解決済みエンティティ(person / subject / project)を入れない(SHARD-RT-08 §placement 原則)
...
- [ ] 5.1 喪失 leaf 専用 spool ストア(`spool:<failover-id>` SQLite)を実装、行は `spool_seq INTEGER PRIMARY KEY AUTOINCREMENT` + `identity_key` + `canonical_json` + `id` + `published` + `recorded_at` + `observation_json`(SHARD-RT-05、R13、R15)
- [ ] 5.2 spool は append-only、`identity_key` UNIQUE / `append_seq` 単調は持たない(spool 自体は dedup しない)(SHARD-RT-05)
- [ ] 5.3 `failover` / `recover` event を partition log の control-plane `event_seq` で記録(SHARD-RT-04、R5)
- [ ] 5.4 drain: `ORDER BY spool_seq` で 1 件ずつ rehome mode (a)(stored identity_key を信頼して現 tree へ再 route → 着地 leaf 内部 append)(SHARD-RT-05 §drain)
- [ ] 5.5 fresh ingest 経路は通さない(R1) + drain 後 spool を retire
- [ ] 5.6 transient dup が排出時に exact index で決定的に除去される property test(SHARD-RT-05 §最終状態決定的)
...
- [ ] 7.1 CDC / Merkle content-model を新 schema として定義
- [ ] 7.2 既存 revisioned snapshot との共存ポリシー
