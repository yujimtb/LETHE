...
**Source:** [sharding_refactor.md](../../../sharding_refactor.md) — D0〜D12 すべて LOCKED の決定台帳
...
| 固定化 | 現状 | 問題 |
| --- | --- | --- |
| dedup 意味論 | `idempotencyKey` は free-form / optional ([src/lake/ingestion.rs:228](../../../src/lake/ingestion.rs))、確率的(SimHash + Bloom)前提が draft に残存 | 同 channel:ts の編集が silent drop / 偽 merge を起こす |
| identity key 構成 | content hash 非含有([src/adapter/slack/mapper.rs:149](../../../src/adapter/slack/mapper.rs)) | Slack 編集を「duplicate」で捨てる(silent 欠損) |
| 衝突判定 | full observation(reactions / meta 含む)を比較([src/lake/store.rs:151](../../../src/lake/store.rs)) | reactions の独立変化で偽 `Conflict` |
| 永続層 | in-memory `LakeStore`(Vec + HashMap)が authoritative ([src/lake/store.rs:53](../../../src/lake/store.rs))、SQLite は補償ロールバック | 多 leaf で RAM に収まらず破綻 |
| watermark cursor | グローバル Vec への `usize` position([src/propagation/watermark.rs:13](../../../src/propagation/watermark.rs)) | 分割するとグローバル position が消える |
| leaf 解決 | 単一 Vec の線形 filter([src/lake/store.rs:101](../../../src/lake/store.rs))、resolver 未実装 | 分割不能 |
| placement | 固定ビット trie / SimHash routing(draft) | route がタイミング依存になり dedup が割れる |
| migration primitive | fresh ingest が `new_id()` + 新 `recordedAt` を毎回付与([src/lake/ingestion.rs:57](../../../src/lake/ingestion.rs)) | split / failover drain / blue-green で identity と propagation cursor が壊れる |

本 change は、[sharding_refactor.md](../../../sharding_refactor.md) §2 の D0〜D12 を normative な仕様要件として確定し、**System Laws(特に Idempotency / Replay / Append-Only)を強化**するためのリファクタリング要件を定義する。
...
詳細根拠は [sharding_refactor.md](../../../sharding_refactor.md) §1.1 の対照表を参照。
