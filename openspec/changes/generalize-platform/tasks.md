...
- [ ] 1.1 scope 付き token 認証を API 層に導入(`/health` 除く)
- [x] 1.2 capability model と scope のマッピングを M08 に追記
- [ ] 1.3 `GET /public/blobs/{sha256}` を署名付き URL 化
...
- [ ] 2.1 `SourceReader` / `Observer` / `WriteBackAdapter` trait を定義
...
- [x] 5.4 `AppService`、Notion client、SQLite persistence を責務別モジュールへ分割する
...
- [ ] 5.8 fmt / build / test / dependency check / document link check / OpenSpec verify を通過させる
