...
| アダプタ | Slack / Google Slides / Notion がコードに直結 | 新 source 追加が trait 実装ではなくコア改修になる |
...
- Affected code: 旧 `src/` 全域(workspace 分割)、`Cargo.toml`,
  `scripts/public_release_audit.py`(クロスプラットフォーム化)
- Breaking changes: **BREAKING** `/api/persons/*` ルートを削除し、
  `/api/projections/{id}/*` のみを提供する
...
Phase 5: workspace ownership remediation + repository layout ← 完了表示と実体の一致
```
