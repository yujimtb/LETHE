...
OEL の索引対象列は correlation_id / causation_id / event_type / stream に occurred_at(時刻レンジ)を加えた最小集合 SHALL とし、これらを JSON 内埋め込みだけでなく列・複合 index として持 SHALL つ。監査 trace が台帳量に比例して数分かかること(実測 Nanihold で 3〜6 分)を、索引化によって解消 SHALL する。canonical append は append-only ゆえ索引は後付け可能であり、最小集合を超える索引列は実需要に駆動されて拡張 SHALL し、投機的に全列を索引化 SHALL NOT する。canonical archive / replay 契約は変更 SHALL NOT する。

#### Scenario: 最小集合の索引で監査 trace が有界
- **WHEN** 既知 correlation / causation / event_type / stream / occurred_at レンジで監査 trace を辿る
- **THEN** 応答時間は台帳全量に比例せず返却件数に対して有界である

#### Scenario: 索引の需要駆動拡張
- **WHEN** 最小集合を超える列での検索需要が生じる
