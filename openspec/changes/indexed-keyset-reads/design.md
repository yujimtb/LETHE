...
- schema は v11 (`indexed_keyset_reads`) とする。SQLite は fresh v11 の初期 DDL に actor_id / causation_id / correlation_id 列と、stream / correlation / causation / event_type / actor / occurred_at / stream+occurred_at の cursor 索引を含め、v9・v10 からの移行では同じ DDL を冪等に適用して scalar 列を event_json から backfill する。PostgreSQL も同じ列・索引・既存行 backfill を行う。OperationalEventStore の filter query は SQLite/PostgreSQL 両方で read pool 経由にする。
- 共通 cursor は scope と persisted sort boundary を JSON 化して URL-safe base64（padding 無し）にした opaque token である。API は token の内部形式を開示せず、scope 不一致・不正 token はエラーにする。projection の同一 sort key 境界は storage 内部で `sort_key` と `item_key` の複合境界として扱う。
- 新しい keyset 契約は `/api/v2` に限定する。`/api/v2/projections/proj:person-page/records` は人物一覧、`.../records/{person_id}` と `/slides`・`/messages`・`/timeline` は人物の cursor page、`/api/v2/projections/claim-queue`・`card-queue`・`reply-slo` は各 persisted projection page、`/api/v2/projections/proj:corpus/records` は search index の keyset page を返す。旧 `/api/...` は凍結し、旧 cursor の意味を変更しない。
- exact 検索は `/api/v2/projections/proj:corpus/exact` の indexed field (`record_id`, `source_object_id`, `thread_key`, `session_id`, `source_type`, `container`) に限定する。safe literal n-gram を持たない regex は `POST /api/v2/projections/proj:corpus/grep` が 202 と job id を返し、`GET /api/v2/search-jobs/{job_id}` で `queued` → `running` → `completed` / `failed` を取得する。
- 可視 blob 表は projection item の insert/update/delete および staging publish の同一 SQLite transaction 内で更新する。consent により可視でない item は projection item 自体を materialize せず、blob 表を ground truth にしない。
- `sync_metrics` の `source_instance = all` がない起動は health に明示的な degraded error を載せる。正常な persisted row がある場合は completed_at・error・累積 metrics を AppCore に復元する。
...
## Known limitations / 既知の限界

- claim/card のページ内フィルタは、現時点では有界な作業キューを前提に、persisted sort key の候補を順に読み取りながらフィルタする保守的な暫定策である。したがって厳密な O(k) は未達であり、候補数が増えると返却件数 k を超えて読み取る。
- PostgreSQL の schema-init は起動ごとに `actor_id` / `causation_id` / `correlation_id` の `UPDATE ... WHERE ... IS NULL` backfill を実行するため、既存行が多い場合は全表走査になる。
- 統一 cursor は共通の opaque token 契約を持つが、API ごとの応答 envelope・page field 名はまだ一様ではなく、応答形状にばらつきが残る。
- regex search job はプロセス内メモリの bounded queue であり、再起動時に queued/running/result を永続復元しない。再起動で未完了 job は失われる。
