...
F1フォローアップとして、consent判定とretraction判定を同じ `opted_out` 集合へ畳み込まない。subject / identifierごとに最新consentを `(published, recorded_at, observation_id)` で保持し、`unrestricted` の到着時は reverse index から対象Observationを引き直して増分再 materialize / 再 index する。retractionは別の恒久遮蔽集合として保持し、consentが `unrestricted` に戻っても解除しない。capture gateは同じ順序関数を共有し、projectionと決定的に一致させる。検索indexの既存doc削除経路に加えて、consumer catch-upがreverse indexから過去Observationを拾い直す経路を使う。reverse indexの永続形状はv13 migrationで追加し、base DDLへは追加しない。

通信projectionも同じ永続化境界を持つ。`consent_by_subject` / `consent_by_identifier` と privacy reverse key は materialization manifest に保存し、canonical Observation 本体だけを in-memory cache として再起動時に再構築する。通信manifestの format version はこの境界変更で 9 から 10 へ上げる。旧versionは新serde型へfull deserializeせず、version判定で background rebuild に送る。旧v9のprivacy field名が残っていてもserde境界で起動を失敗させない。incremental consumer は `opted_out` と `unrestricted` の双方で `observations_for_privacy_key` を再pullしてからfoldし、再pull前でも persisted observation key/fact を除去できなければならない。
