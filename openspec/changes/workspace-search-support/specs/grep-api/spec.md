...
**Module:** (new) grep-api
**Scope:** Access Controlled Corpus Projection 上の正規表現 grep API
**Dependencies:** M01 Domain Kernel, M05 Projection Engine, M14 API Serving, corpus-projection (本 change)
**Agent:** Spec Designer (API 契約) → Implementer (grep engine + endpoint) → Reviewer (regex safety + match completeness)
