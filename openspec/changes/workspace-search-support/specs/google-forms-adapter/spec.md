# Spec Delta: google-forms-adapter

**Change:** workspace-search-support
**Module:** (new) google-forms-adapter
**Scope:** Google Forms source adapter — 構造 + 回答事実 + 回答内容の分離
**Dependencies:** M01 Domain Kernel, M02 Registry, M03 Observation Lake, M08 Governance, M09 Adapter Policy, M11 Google Slides Adapter (schema 参照), corpus-projection (本 change)
**Agent:** Spec Designer (capture 仕様 + 回答分離) → Implementer (API client + adapter) → Reviewer (回答内容非露出検証)
