...
source sync と supplemental write は derived projection lane を取得した後にnon-bulk projection operationと短いbulk-session admission handshakeを取得し、bulk sessionがinactiveであることを確定しなければならない。derived laneの待機中は `non_bulk_projection_operation` と `bulk_import_operation` のどちらも保持してはならない。supplemental writeはユーザー書込なのでrebuild中にskipせずderived laneを待ち、lane取得後にbulk-session admissionを再検査する。bulk session beginは実行中のnon-bulk operationを待たずconflictとしてfail-fastにするが、derived laneを待つだけのsync/supplementalをactive operationとみなしてはならない。
