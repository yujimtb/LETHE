...
source sync は background non-corpus rebuild が進行中かをoperation lock取得前に検査し、進行中なら `sync_skip_reason="background_non_corpus_rebuild"` をlogしてcycleを成功扱いで即時skipしなければならない。skipはhealthのlast sync、persisted sync state、source cursorを更新してはならず、rebuild完了後の次回scheduleは通常どおりsyncを実行しなければならない。

source sync と supplemental write は derived projection lane を取得した後にnon-bulk projection operationを取得し、bulk-session admissionのB=`bulk_import_operation`はblocking取得せず`try_lock`で検査しなければならない。BがbusyならN=`non_bulk_projection_operation`とD=`derived_projection_lane`を解放しなければならず、Dを保持したままBを待機またはbackoffしてはならない。syncは`sync_skip_reason="bulk_import_operation_active"`でcycleを成功扱いにし、supplemental writeは全guard解放後だけ上限付きbackoffで再試行し、上限到達時は明示conflictを返さなければならない。derived laneの待機中はNとBのどちらも保持してはならない。bulk session beginは実行中のnon-bulk operationを待たずconflictとしてfail-fastにするが、derived laneを待つだけのsync/supplementalをactive operationとみなしてはならない。

bulk-session appendはBを保持してsession照合、canonical append、persisted target watermark更新、first-append/consent materializationのD取得まで直列化しなければならない。first appendのstale publication前に後続session appendを割り込ませてはならず、consent capture順はcanonical append順と一致しなければならない。B/D間の唯一のblocking edgeはB→Dでなければならない。

skipした`SyncReport`は`skipped=true`と保存済みの直近`last_sync_at`（実績がなければ`None`）を返し、新しいtimestampを生成してはならない。通常完了は`skipped=false`と今回のcompletion timestampを返さなければならない。

migration/recovery rebuild中のbulk session beginはsession metadataだけを永続化し、derived projectionを変更しないため、rebuildが実際にin-flightである間に限りstale projection catalogを理由に拒否してはならない。rebuildが進行していないstale projectionは従来どおりfail-fastしなければならない。
...
#### Scenario: rebuild中のsyncをskipしてbulk session beginを許可する

- **WHEN** staleなnon-corpus projectionを復旧するbackground rebuild中にscheduled sync cycleが開始する
- **THEN** syncは1秒以内に成功扱いでreturnし、`sync_skip_reason="background_non_corpus_rebuild"`をlogする
- **AND** last sync、persisted sync state、source cursorを更新しない
- **AND** bulk session beginは`bulk_import_non_bulk_projection_active`またはprojection staleを返さず成功する
- **AND** rebuild完了後の次回syncは通常実行してcompleted sync stateを永続化する

#### Scenario: supplemental writeのrebuild待機はbulk beginを占有しない

- **WHEN** supplemental writeがbackground rebuildのderived laneを待つ
- **THEN** 待機中はnon-bulk projection operationを保持しない
- **AND** lane取得後にbulk sessionがactiveなら明示conflictを返し、inactiveなら通常どおりsupplementalとprojectionをatomic commitする

#### Scenario: first appendとsyncのB/D交差は循環しない

- **WHEN** bulk sessionのfirst appendがBを保持してtarget watermarkを永続化し、D取得直前で停止している
- **AND** source sync cycleがDを取得してbulk-session admissionを検査する
- **THEN** syncはBを待たず1秒以内に`bulk_import_operation_active`理由でskipし、D/Nを解放する
- **AND** first appendはDを取得して5秒以内に完了し、first-append publishは一回、consent stateとsession targetはcanonical append順に一致する
- **AND** skipped reportは直前のlast sync timestampを保ち、healthまたはpersisted sync stateを進めない
